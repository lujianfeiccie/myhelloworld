...
<div style="height:100%; text-align:center;">
  <table width="100%"  style="height:100%; text-align:center;" cellpadding="0" cellspacing="0">
    <tr style="background-image:url(images/bg.gif); height:27px;">
      <td style="background-image:url(images/bg.gif); height:27px; text-align:center;"><span class="title" id="list_title"><strong>新书列表</strong></span><br /></td>
    </tr>
    <tr>
        ...
        <div class="item" style="display:none;">
          标题：<input size="50" name="title" id="title" type="text"  value="" />
           ...
           <label style="display:none;"  runat="server" id="hcon"></label>
            ...
            <thead><tr><th title="单击按标签名排序" style="width:120px;">书名</th><th title="类别" style="width:80px;">类别</th><th title="发布作者排序" style="width:60px;">作者</th><th title="发布时间排序" style="width:60px;">出版时间</th><th title="出版社"  style="width:60px;">出版社</th><th style="width:80px;">推荐操作</th><th style="width:80px;">更改</th><th style="width:50px;">删除</th></tr>
            </thead><tbody></HeaderTemplate>
            <ItemTemplate> 
          <tr style="line-height:15px;" id="tr<%# DataBinder.Eval(Container.DataItem,"id") %>"><td id="title<%# DataBinder.Eval(Container.DataItem,"id") %>"><input type="checkbox" value="<%# DataBinder.Eval(Container.DataItem,"id") %>" name="choosecheck" /><a href="../BookView.aspx?id=<%# DataBinder.Eval(Container.DataItem,"id") %>" target="_blank"><%# DataBinder.Eval(Container.DataItem,"title") %></a>  </td>
           <td id="type<%# DataBinder.Eval(Container.DataItem,"id") %>"><%# DataBinder.Eval(Container.DataItem,"typename") %></td>
           <td id="use<%# DataBinder.Eval(Container.DataItem,"id") %>"><%# DataBinder.Eval(Container.DataItem,"author") %></td>
           <td id="times<%# DataBinder.Eval(Container.DataItem,"id") %>"><%# DataBinder.Eval(Container.DataItem,"pub_time") %></td>
            <td id="pub<%# DataBinder.Eval(Container.DataItem,"id") %>"><%# DataBinder.Eval(Container.DataItem,"publisher") %></td>
          
          
              <td id="check<%# DataBinder.Eval(Container.DataItem,"id") %>"><a href="#"  style="cursor:pointer;" onclick="checkbox('<%# DataBinder.Eval(Container.DataItem,"id") %>','<%# DataBinder.Eval(Container.DataItem,"istop") %>');"><img  width="60" title="点击进行推荐操作" style="border:none;" src="images/top_<%# DataBinder.Eval(Container.DataItem,"istop") %>.gif" /></a></td>
              <td id="change<%# DataBinder.Eval(Container.DataItem,"id") %>"><a href="pubBook.aspx?act=edit&id=<%# DataBinder.Eval(Container.DataItem,"id") %>"  style="cursor:pointer;" >更改</a></td>
              <td><a href="pubBook.aspx?act=delete&id=<%# DataBinder.Eval(Container.DataItem,"id") %>" >删除</a></td></tr>
            </ItemTemplate>
...
&nbsp;更改成状态<select style="width:80px;" id="shenhe" name="shenhe"><option value="1">置顶</option>
             <option value="0">撤下</option></select>
    &nbsp;&nbsp;<input type="button" onclick="checkall();" value="置顶/撤下" />&nbsp;&nbsp;<input onclick="delall();" type="button" value="执行删除" />
...
function checkbox(id,type)
{
    ...
    if(confirm("你确认要 "+change+" ID为 "+id+" 的 书籍 记录吗？")==true)
    {
         $.ajax({ type:"Get",
           url: "EditBook.aspx",
           cache: false,
           ...
           success: function(data){
                  if(data=="ok")
                  {                     
                    alert("已成功 "+change+" ID为 "+id+" 的 书籍 记录！"); 
                     location.href=jumpurl;
                  }
                   else
                   {
                   alert(change+" ID为 "+id+" 的 书籍 记录失败，请重试！/n 错误类型："+data); 
                   }    
           }
        }); 
    }
